defmodule Mix.Tasks.Korangar.Packets.Gen.Json do
  ...
  defp convert_to_simple_json(%{"crate_version" => version, "index" => index}) do
    ...
    %{
      version: version,
      structs: clear_list(structs),
      inline_structs: clear_list(inline_structs),
      keyless_structs: clear_list(keyless_structs),
  ...
  defp extract_type(%{"resolved_path" => %{"args" => type, "id" => id, "path" => name}}) do
    %{String.to_atom(name) => %{id: id, of: extract_type(type)}}
  end
  ...
  defp extract_type(%{"array" => %{"len" => length, "type" => type}}) do
    %{Array: %{of: extract_type(type), length: String.to_integer(length)}}
  end
